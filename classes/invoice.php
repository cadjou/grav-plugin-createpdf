...
function sizeOfText( $texte, $largeur )
{
	...
	while ( $loop )
	{
		...
		$res = 1 + ($largeur ? floor( $length / $largeur) : 0) ;
		$nb_lines += $res;
	}
	return $nb_lines;
}
...
function addSociete( $nom, $adresse , $pathlogo = '')
{
	$x1 = 10;
	$y1 = 8;
	//Positionnement en bas
    if (realpath($pathlogo))
    {
        list($width, $height, $type, $attr) = getimagesize($pathlogo);
        $this->Image($pathlogo,$x1,$y1,30);
        $y1 = round($height/10,0) - 5;
    }
	$this->SetXY( $x1, $y1 );
